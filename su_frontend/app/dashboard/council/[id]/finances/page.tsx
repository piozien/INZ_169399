...
export default function FinancesPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!budget) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] text-txtcolor-300 gap-6 p-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="p-6 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border pb-6">
                ...
                <div className="flex gap-3 flex-wrap items-center">
                    ...
                    <button onClick={() => handleDownload('xlsx')} className="flex items-center gap-2 px-4 py-2 bg-secondarybg border border-border hover:bg-inputbg rounded-lg text-sm transition-colors text-foreground" title="Pobierz Excel">
                        <FileDown className="h-4 w-4" /> <span className="hidden sm:inline">Excel</span>
                    </button>

                    {(canEditBudget || canDeleteBudget) && <div className="w-px h-8 bg-border mx-2 hidden sm:block"></div>}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div className="bg-secondarybg/50 p-6 rounded-xl border border-success/20 flex items-center justify-between shadow-sm">
                    <div>
                        <span className="text-sm font-bold text-success uppercase tracking-wider">Przychody</span>
                        <span className="block text-2xl font-bold mt-1 text-success">+{budget.totalIncome?.toFixed(2) || '0.00'} PLN</span>
                    </div>
                    <div className="p-3 bg-success/10 rounded-full text-success"><TrendingUp className="h-6 w-6" /></div>
                </div>

                <div className="bg-secondarybg/50 p-6 rounded-xl border border-error/20 flex items-center justify-between shadow-sm">
                    <div>
                        <span className="text-sm font-bold text-error uppercase tracking-wider">Wydatki</span>
                        <span className="block text-2xl font-bold mt-1 text-error">-{budget.totalExpenses?.toFixed(2) || '0.00'} PLN</span>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-secondarybg rounded-xl border border-border overflow-hidden flex flex-col h-auto max-h-[500px] shadow-sm">
                    <div className="p-4 border-b border-border bg-inputbg/30 flex justify-between items-center backdrop-blur-sm shrink-0">
                    ...
                    <div className="flex-1 overflow-y-auto scrollbar-thin p-0">
                        {transLoading ? (
                            <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin text-primary h-8 w-8" /></div>
                            ...
                            <table className="w-full text-sm text-left border-collapse">
                                <thead className="text-xs text-txtcolor-300 uppercase bg-inputbg/50 sticky top-0 backdrop-blur-md z-10">
                                <tr>
