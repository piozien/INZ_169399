...
export default function MemberCard({ member, canManage, onEdit, onDelete, onClick }: Props) {
    ...
    return (
        <div
            ...
            className="group relative bg-secondarybg border border-border rounded-xl p-5 transition-all duration-300 hover:border-secondary cursor-pointer flex flex-col items-center text-center"
        >
        ...
        {member.roleName || member.role}
            ...
            {canManage && (
                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-200 flex gap-2 transform translate-y-[-5px] group-hover:translate-y-0">
                    <button
