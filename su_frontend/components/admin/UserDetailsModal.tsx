...
export default function UserDetailsModal({ isOpen, onClose, user }: Props) {
    ...
    const { assignRole, removeRole, deleteUser, unblockUser, availableRoles } = useAdminUsers();
    const { data: logs, isLoading: logsLoading } = useUserLogs(isOpen ? user.id : null);

    if (!isOpen) return null;

    return (
        <div className="bg-background/80 animate-in fade-in fixed inset-0 z-[60] flex items-center justify-center p-4 backdrop-blur-sm duration-200">
            <div className="bg-background border-border animate-in zoom-in-95 flex max-h-[90vh] w-full max-w-2xl flex-col rounded-2xl border shadow-2xl">
                <div className="border-border bg-secondarybg/20 flex items-start justify-between border-b p-6">
                    <div>
                        ...
                        <div className="text-txtcolor-300 mt-2 font-mono text-[10px] tracking-widest uppercase opacity-50">
                            ID: {user.id}
                        </div>
                    </div>
                    <button
                    ...
                    {tab === 'ROLES' && (
                        <div className="animate-in slide-in-from-left-2 fade-in space-y-8 duration-300">
                                            ...
                                            >
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if (
                                                            confirm(
                                                                `Czy na pewno odebrać rolę ${role}?`
                                                            )
                                                        )
                                                            removeRole({
                                                                userId: user.id,
                            ...
                            <div className="border-border border-t pt-6">
                                ...
                                <div className="text-txtcolor-300 bg-secondarybg/30 border-border flex items-start gap-2 rounded border p-2.5 text-[10px]">
                                    ...
                                    {user.status === 'BLOCKED' ? (
                                        ...
                                        <button
                                            onClick={() => {
                                                if (
                                                    confirm(
                                                        'Czy na pewno zablokować/usunąć tego użytkownika?'
                                                    )
                                                ) {
                                                    deleteUser(user.id);
                                                    onClose();
                        ...
                        <div className="animate-in slide-in-from-right-2 fade-in duration-300">
                            {logsLoading ? (
                                ...
                                <div className="space-y-3">
                                    {logs?.length === 0 && (
                                        <div className="text-txtcolor-300 flex flex-col items-center py-10 text-center opacity-50">
                                    ...
                                    {logs?.map((log) => (
                                        ...
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className="text-foreground bg-background border-border rounded border px-2 py-0.5 text-[10px] font-bold tracking-wide uppercase shadow-sm">
                                                    {log.actionType}
                                                </span>
